...
## 2. Surgical Modifications & Archiving
* **Rule:** Do not write or replace whole files unless they are very small (<50 lines). Avoid reading or writing multiple files individually when transferring, backing up, or archiving large datasets.
* **Method:** Use targeted text replacement tools (\`replace_text\`, \`patch_file\`, \`replace_file_content\`, \`multi_replace_file_content\`) to apply edits to specific lines only. For bundling, transferring, or backing up directories or groups of files, use the \`compress_files\` and \`decompress_files\` tools.
* **Cost Impact:** Reduces output token generation by up to 95% and avoids round-trip context bloat from bulk individual file operations.
